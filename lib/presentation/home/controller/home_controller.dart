...
class HomeController extends GetxController {
  ...
  Future getWeatherData() async {
    try {
      var request = http.Request(
        'GET',
        Uri.parse(
            'https://api.openweathermap.org/data/2.5/weather?lat=30.0686417&lon=31.2112086&appid=f4f8c17a28834dc12689b7bb28fc68e0'),
      );
  ...
  Future getHourWeatherData() async {
    try {
      var request = http.Request(
        'GET',
        Uri.parse(
            'https://api.open-meteo.com/v1/forecast?latitude=30.0686417&longitude=31.2112086&hourly=temperature_2m,relative_humidity_2m&temperature_unit=fahrenheit&timezone=GMT&forecast_days=3'),
      );
      http.StreamedResponse response = await request.send();
      if (response.statusCode == 200) {
        var responseJson = jsonDecode(await response.stream.bytesToString());
  ...
  String getWeatherIcon({required int weatherCode}) {
    ...
    if (weatherCode == 801) {
      ...
      weatherCondition = 'assets/icons/default.png'; // Default icon
    }
